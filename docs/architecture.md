...
Обновление ОС не привязано к дистрибутиву в инвентаре: remote wrapper определяет пакетный менеджер во время запуска. При отсутствии `flock` используется атомарный lock directory.
