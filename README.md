...
- безопасное обновление пакетов Linux и 3x-ui с резервным копированием;
...
Интерактивная установка автоматически создаёт конфигурацию, SSH-ключи и инвентарь для любого количества серверов:
...
Для полностью ручной настройки используйте:
...
Поддерживаются Debian/Ubuntu (`apt-get`), Oracle Linux/RHEL/Rocky/Alma (`dnf` или `yum`), openSUSE/SLES (`zypper`), Alpine (`apk`) и Arch (`pacman`). Полная подготовка SSH-пользователей, Telegram и панелей описана в [инструкции установки](docs/installation.md).
